        ...
        <div v-for="review in reviews" :key="review.id" class="mb-6 pb-6 border-b border-gray-200 last:border-b-0">
          ...
          <div v-if="review.showComments" class="mt-4 space-y-2">
            <div v-for="comment in review.comments" :key="comment.id" class="bg-gray-50 p-2 rounded">
              <p class="text-sm text-[#4A524D]"><span class="font-semibold">{{ comment.name }}:</span> {{ comment.content }}</p>
            </div>
            <form @submit.prevent="addComment(review.id)" class="flex mt-2">
              <input v-model="newComment" type="text" placeholder="Add a comment" class="flex-grow px-3 py-2 border border-gray-300 rounded-l-md focus:outline-none focus:ring-2 focus:ring-[#44AAE2]">
              <button type="submit" class="bg-[#44AAE2] text-white px-4 py-2 rounded-r-md hover:bg-[#115583]">Post</button>
            </form>
          </div>
        </div>
      </div>
