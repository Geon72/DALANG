    ...
    <div class="mx-auto mb-9">
      ...
      <Splide :options="splideOptions" aria-label="Home Page Banner Carousel">
        ...
        <SplideSlide v-for="(slide, index) in slides" :key="index">
          <div class="relative h-96 overflow-hidden rounded-lg" :style="{ backgroundColor: slide.color }">
            <!-- 어두운 배경 오버레이 -->
            <div class="absolute inset-0 bg-black bg-opacity-40 transition-opacity"></div>
            <!-- 슬라이드 콘텐츠 -->
            <div class="absolute inset-0 flex flex-col items-center justify-center text-center space-y-4">
              <h2 class="text-white font-bold text-2xl sm:text-3xl lg:text-4xl tracking-wide drop-shadow-lg">
                {{ slide.title }}
              </h2>
              <button v-if="slide.buttonText === 'Click!'" @click="openCookieClickerView"
              ...
              <button v-else-if="slide.buttonText === 'Explore!'" @click="openPachinkoView"
                class="bg-white font-bold py-3 px-6 sm:py-4 sm:px-8 lg:py-5 lg:px-10 rounded-full text-lg sm:text-xl lg:text-2xl shadow-xl transform transition-transform hover:scale-105 hover:bg-opacity-95"
                :style="{ color: slide.color }" :aria-label="'Go to ' + slide.buttonText">
                {{ slide.buttonText }}
              </button>
