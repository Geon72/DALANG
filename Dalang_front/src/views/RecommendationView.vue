  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <template v-if="currentTab === '정기예금'">
        <RecommendationFilterSectionDeposit :banks="banks" :periods="periods" :subscriptionMethods="subscriptionMethods"
          @search="searchDeposits" />
        <RecommendationResultTableDeposit :tableHeaders="depositTableHeaders" :products="depositProducts"
          :sortOrder="sortOrder" />
      </template>

      <!-- 정기적금 컴포넌트들 -->
      <template v-else>
        <RecommendationFilterSectionSaving :banks="banks" :periods="periods" :subscriptionMethods="subscriptionMethods"
          @search="searchSavings" />
        <RecommendationResultTableSaving :tableHeaders="savingTableHeaders" :products="savingProducts"
          :sortOrder="sortOrder" />
      </template>
    </div>
  </div>
